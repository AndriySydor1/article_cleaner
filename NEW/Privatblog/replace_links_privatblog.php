...
$sql = "SELECT id, `fulltext`, `introtext` FROM stasz_content WHERE `fulltext` LIKE '%http%' OR `introtext` LIKE '%http%'";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($cleaned_fulltext !== $row["fulltext"] || $cleaned_introtext !== $row["introtext"]) {
            $stmt = $conn->prepare("UPDATE stasz_content SET `fulltext` = ?, `introtext` = ? WHERE id = ?");
